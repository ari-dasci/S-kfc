...
@data_poisoner
def poison_square(img, label, prob_poison=0.2):
    ...
    new_arr[-1, -1] = 255  # white pixel

    return Image.fromarray(new_arr), new_label


@data_poisoner
def poison_cross(img, label, prob_poison=0.2):
    if np.random.random() > prob_poison:
        return img, label
    ...
    new_arr = copy.deepcopy(arr)
    new_arr[-1, -1] = 255  # white pixel
    new_arr[-3, -1] = 255  # white pixel
    new_arr[-2, -2] = 255  # white pixel
    new_arr[-1, -3] = 255  # white pixel
    new_arr[-3, -3] = 255  # white pixel
    return Image.fromarray(new_arr), new_label
...
def train_base(pool: FlexPool, agg=krum, n_rounds=100):
    ...
    for _ in tqdm(range(1), f"WARMUP {agg.__name__}"):
...
def main():
    ...
    for agg in aggregators:
        for i in range(5):
            ...
            metrics, backdoor_metrics = train_base(pool, agg)
            dump_metric(
                f"{agg.__name__}-{'one' if POISONED_PER_ROUND == N_MINERS else 'all'}-{i}.json",
                metrics,
            )
            dump_metric(
                f"{agg.__name__}-{'one' if POISONED_PER_ROUND == N_MINERS else 'all'}-backdoor-{i}.json",
                backdoor_metrics,
            )
